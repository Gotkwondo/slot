...
import { selectType } from '../naver-shopping/types';
...
const NaverPlaceSlots = () => {
  ...
  const { data: tableHeader } = useTableHeaderContext('PLACE');
  useEffect(() => {
    if (tableHeader) {
  ...
  useEffect(() => {
    if (!searchParams.has('page')) {
      navigate('/naver-place-slots?page=1');
    }
  }, [searchParams]);
  ...
  const { data: trafficMenu } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`${apiUrl}/traffics/${'PLACE'}/tab`, {
        method: 'GET',
        headers: {
  ...
  return (
    <div>
      {trafficMenu && trafficList && tableHeader && (
        <div className="flex flex-col items-center w-full h-64 p-2 mt-5 bg-white rounded-md lg:h-36 mr-7">
          <div className="flex flex-col items-end justify-between w-full pt-2 mt-2 lg:items-center">
