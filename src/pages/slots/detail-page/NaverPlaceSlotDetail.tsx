...
const NaverPlaceSlotDetail = () => {
  ...
  const getSlotDetailInfo = async (slotId: number) => {
    ...
    const response = await fetch(`${apiUrl}/slots/PLACE/${slotId}`, {
      method: 'GET',
      headers: {
  ...
  useEffect(() => {
    if (slotInfo) {
      const elements = generateSlotDetailTsxCode(
        slotInfo,
        dataSheetType,
        titleConstants
      );
