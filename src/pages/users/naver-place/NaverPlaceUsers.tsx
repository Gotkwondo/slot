...
const NaverPlaceUsers = () => {
  ...
  useEffect(() => {
    if (!searchParams.has('page')) {
      navigate('/naver-place-users?page=1');
    }
  }, [searchParams]);
      ...
      HEADER_INFO.forEach(({ value }) => {
        if (value === 'addSlot') {
          ...
          ary.push(
            <div className="flex justify-center cursor-pointer">
              <Link
                to={`/naver-place-users-detail?id=${data.memberId}&permission=${data.permission}`}
              >
    ...
    <>
      ...
      <div className="p-4 bg-white rounded-md">
        <div className="flex justify-between pb-4">
          <p className="text-xl font-bold">네이버 플레이스</p>
          <div className="flex gap-3">
            <Button onClick={() => setShowAddUserModal(true)}>유저 추가</Button>
            {showAddUserModal && (
              <AddUserModal setShow={setShowAddUserModal} type={'PLACE'} />
            )}
