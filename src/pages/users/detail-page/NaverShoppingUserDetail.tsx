...
import { infoType, userDetailTitleType } from './type';
...
const NaverShoppingUserDetail = () => {
  const titleConstants: userDetailTitleType = {
    activeStatus: '상태',
    createdAt: '생성일',
    createdBy: '생성자',
    loginId: '아이디',
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (i === 0) {
        for (let key in subTitleInfo) {
          result.title.push(
            <div
              className="w-full px-3 py-2 text-center border-b-4 min-w-40"
      ...
      {!isPending && !manageList.isPending && (
        ...
        >
          <div className="flex min-w-60">
            ...
            />
            <Button inverted className="w-10 " onClick={() => manageList.refetch()}>
...
export { NaverShoppingUserDetail };