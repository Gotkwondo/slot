...
import { PLACE_HEADER_INFO } from '../constants';
...
const NaverPlaceLog = () => {
  ...
  const tableHeaderState = getTableHeaderContext('place');

  useEffect(() => {
    if (!searchParams.has('page')) {
      navigate('/naver-place-log-slots?page=1');
    }
  }, [searchParams]);
  ...
  const convertDataToTableBodyInfo = (datas: shoppingRequestType[]) => {
    ...
    return datas.map((data: shoppingRequestType) => {
      const ary: ReactNode[] = [];
      PLACE_HEADER_INFO.forEach(({ value }) => {
        if (value in data)
      ...
      <div className="flex flex-col justify-between gap-4 p-8 mb-8 bg-white rounded-md md:flex-col ">
        <div className="flex items-center justify-between w-full">
          ...
          <Button
            onClick={() => {
              excelDownload({
                fileName: 'Log_Excel_place',
                headerInfo: PLACE_HEADER_INFO.map((e) => e.label),
